...
def backup_database(src_path: Path, dest_dir: Path, label: str) -> Path:
    ...
    try:
        ...
        backup = dest_conn.backup(src_conn, pages=0)  # pages=0 = all at once
        backup.finish()
    finally:
        src_conn.close()
        dest_conn.close()
...
def report_metrics(label: str, backup_path: Path, ok: bool) -> None:
    ...
    # The Prometheus registry is not typically available from scripts.
    # Log a structured message that a sidecar or log shipper can parse.
