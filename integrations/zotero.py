...
class ZoteroSync:
    ...
    def search_local(self, query: str) -> list[ZoteroItem]:
        ...
        for item in self.local_items.values():
            if (
                ...
                or any(query_lower in a.lower() for a in item.authors)
                or query_lower in (item.abstract or "").lower()
    ...
    def add_item_from_agent(
        self, title: str, authors: list[str], year: int, doi: str | None = None, **kwargs
    ) -> str | None:
        ...
        template = {
            ...
            "title": title,
            "creators": [
                {"creatorType": "author", "firstName": "", "lastName": a} for a in authors
            ],
            "date": str(year),
            "DOI": doi,
        }
    ...
    def export_bibliography(
        self, name: str, format: str = "biblatex", items: list[str] | None = None
    ) -> str:
