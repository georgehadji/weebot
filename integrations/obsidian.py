...
class ObsidianNote:
    ...
    def to_markdown(self) -> str:
        ...
        if YAML_AVAILABLE:
            ...
            lines.extend(["---", json.dumps(frontmatter, indent=2), "---"])

        lines.extend(["", f"# {self.title}", "", self.content, ""])
...
class ObsidianVault:
    ...
    def create_note(
        self, note: ObsidianNote, folder: str | None = None, inbox: bool = False
    ) -> Path:
    ...
    def generate_from_experiment(self, experiment_id: str, exp_data: dict | None = None) -> Path:
        """Generate note from experiment data"""
        note = ResearchNote(
            title=f"Experiment: {experiment_id}",
            experiment_id=experiment_id,
            tags=["experiment", "active"],
            metadata={"experiment_id": experiment_id, "status": "active"},
        )
...
"""
        ...
        dashboard = ObsidianNote(
            title="Research Dashboard", content=content, tags=["dashboard", "meta"]
        )
    ...
    def sync_with_experiments(self, repro_manager=None, bidirectional: bool = False):
    ...
    def export_for_publication(self, note_title: str, format: str = "markdown") -> str:
