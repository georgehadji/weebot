...
class ParameterResolver:
    ...
    def resolve(
        self, template: WorkflowTemplate, provided: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        for name, schema in template.parameters.items():
            if name in provided:
                ...
                raise ParameterValidationError(
                    f"Required parameter '{name}' not provided", parameter=name
                )

        return resolved

    def _validate_and_convert(self, name: str, value: Any, schema: ParameterSchema) -> Any:
        """
        ...
        if param_type == "enum":
            if schema.enum_values and value not in schema.enum_values:
                raise ParameterValidationError(
                    f"Parameter '{name}' must be one of {schema.enum_values}, " f"got '{value}'",
                    parameter=name,
