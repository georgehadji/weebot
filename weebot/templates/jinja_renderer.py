...
class JinjaTemplateRenderer:
    ...
    def render(self, template_str: str, context: dict[str, Any], safe_mode: bool = True) -> str:
        """
    ...
    def render_workflow(self, workflow: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(workflow, dict):
            return {key: self.render_workflow(value, context) for key, value in workflow.items()}
    ...
    def validate_template(self, template_str: str) -> list[str]:
        ...
        dangerous_patterns = ["{% raw %}", "{% macro", "{% call", "{% do"]
...
class ConditionalWorkflowBuilder:
    ...
    def __init__(self, renderer: JinjaTemplateRenderer | None = None):
        self.renderer = renderer or JinjaTemplateRenderer()

    def build_workflow(
        self, template_workflow: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
        """
            ...
            if condition:
                ...
                try:
                    result = self.renderer.render(
                        f"{{% if {condition} %}}true{{% endif %}}", context
                    )
                    if result != "true":
...
class LoopWorkflowBuilder:
    ...
    def _expand_task_loop(
        self, task_id_template: str, task_def: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for item in iterable:
                ...
                # Render task ID
                task_id = self.renderer.render(f"{prefix}{template}{suffix}", loop_context)
