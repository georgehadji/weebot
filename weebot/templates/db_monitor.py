...
class ConnectionPoolMonitor:
    ...
    def __init__(
        ...
        self._last_saturation_alert: datetime | None = None
    ...
    async def track_connection(self, connection_id: str | None = None):
        """
        ...
        try:
            ...
            async with asyncio.timeout(self.acquisition_timeout):
                ...
                self._connection_metrics[conn_id] = ConnectionMetrics(
                    acquired_at=datetime.now(), acquisition_time_ms=acquisition_time * 1000
                )
            ...
            self._timeouts += 1
            _log.error("Connection acquisition timeout after %.1fs", self.acquisition_timeout)
            raise
    ...
    def record_query_start(
        self, connection_id: str, query_id: str, timeout: float | None = None
    ) -> None:
    ...
    def record_query_end(self, connection_id: str, query_id: str, success: bool = True) -> None:
    ...
    def record_pool_snapshot(
        self, checked_out: int, available: int, waiting: int = 0
    ) -> PoolSnapshot:
        """
    ...
    def _maybe_alert_saturation(self, saturation: float, checked_out: int, waiting: int) -> None:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        recent_saturation = [s.saturation_percent for s in self._snapshots[-10:]]
    ...
    def get_recommendations(self) -> list[str]:
        ...
        if metrics["slow_acquisition_rate"] > 0.1:  # >10% slow
            recommendations.append("Consider increasing pool size: >10% of acquisitions are slow")
...
class MonitoredDatabaseManager:
    ...
    def __init__(self, inner_manager: Any, monitor: ConnectionPoolMonitor | None = None):
