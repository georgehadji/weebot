...
class FeatureConfig:
    ...
    allowed_users: set | None = None  # None = all users
...
def register_default_features():
    ...
    _feature_flags.register(
        FeatureConfig(name="adaptive_suggestions", state=FeatureState.DISABLED)  # Opt-in by default
    )

    _feature_flags.register(FeatureConfig(name="template_composition", state=FeatureState.ENABLED))

    _feature_flags.register(
        FeatureConfig(name="hot_reload", state=FeatureState.DISABLED)  # Dev only
    )

    _feature_flags.register(FeatureConfig(name="execution_analytics", state=FeatureState.ENABLED))
