...
class PrivacyAuditEvent:
    ...
    template_name: str
    user_hash: str
    violation_type: PrivacyViolationType | None = None
...
class PrivacyAuditMiddleware:
    ...
    def allow_collaborative_query(
        self, template_name: str, user_id: str, proposed_user_count: int | None = None
    ) -> bool:
        """
    ...
    def validate_suggestion_result(
        self, template_name: str, user_id: str, result: dict[str, Any]
    ) -> bool:
        """
        ...
        if user_count < self.min_user_count:
            event = PrivacyAuditEvent(
                ...
                details={"actual_count": user_count, "required_count": self.min_user_count},
                blocked=True,
            )
            self._log_event(event)
            self._track_violation()
        ...
        if execution_count < self.min_sample_size:
            event = PrivacyAuditEvent(
                ...
                user_hash=user_hash,
                violation_type=PrivacyViolationType.SMALL_SAMPLE,
                details={"actual_sample": execution_count, "required_sample": self.min_sample_size},
                blocked=True,
            )
            self._log_event(event)
            self._track_violation()
    ...
    def _track_violation(self) -> None:
        ...
        self._recent_violations = [v for v in self._recent_violations if v.timestamp() > cutoff]
    ...
    def get_report(self, template_name: str | None = None) -> PrivacyReport:
        """
    ...
    def get_audit_trail(
        self, template_name: str | None = None, limit: int = 100
...
class ValidatingSuggestionEngine:
    ...
    def __init__(self, inner_engine: Any, audit: PrivacyAuditMiddleware | None = None):
    ...
    async def suggest_parameters(self, *args, **kwargs) -> list[Any]:
        ...
        if template and context:
            ...
            if not self.audit.allow_collaborative_query(
                template_name=template.name, user_id=context.user_id
            ):
