...
from sqlalchemy import Column, String, Integer, Float, DateTime, Boolean, Text, select, func, and_
...
class SuggestionContext:
    ...
    domain_hint: str | None = None  # e.g., "research", "coding"
...
class AdaptiveSuggestionEngine:
    ...
    def _get_cache_key(self, template_name: str, user_id: str, available_params: set[str]) -> str:
    ...
    async def suggest_parameters(
        ...
        params_to_suggest = [
            name for name in template.parameters.keys() if name not in current_input
        ]
        ...
        if self.enable_personal:
            personal = await self._get_personal_suggestions(template, context, params_to_suggest)
        ...
        if remaining and self.enable_collaborative:
            collaborative = await self._get_collaborative_suggestions(template, context, remaining)
    ...
    async def _get_personal_suggestions(
        self, template: WorkflowTemplate, context: SuggestionContext, param_names: list[str]
    ) -> list[ParameterSuggestion]:
        ...
        async with self.db_session_factory() as session:
            for param_name in param_names:
                ...
                stmt = select(
                    UserPreferenceRecord.preferred_parameters_json, UserPreferenceRecord.usage_count
                ).where(
                    and_(
    ...
    async def record_execution_outcome(
        ...
        async with self.db_session_factory() as session:
            ...
            await self._update_user_preferences(session, user_hash, template.name, parameters)

            await session.commit()

            # Invalidate cache
            cache_key = self._get_cache_key(template.name, user_id, set(parameters.keys()))
            self._suggestion_cache.pop(cache_key, None)

    async def _update_user_preferences(
        self, session: AsyncSession, user_hash: str, template_name: str, parameters: dict[str, Any]
    ):
    ...
    async def get_suggestion_stats(self, template_name: str | None = None) -> dict[str, Any]:
        ...
        async with self.db_session_factory() as session:
            ...
            if template_name:
                stmt = stmt.where(ParameterEffectivenessRecord.template_name == template_name)
    ...
    async def purge_old_data(self, days: int = 30):
        ...
        async with self.db_session_factory() as session:
            ...
            stmt = select(UserPreferenceRecord).where(UserPreferenceRecord.last_updated < cutoff)
            result = await session.execute(stmt)
            ...
            _log.info(
                f"Purged {len(old_records)} effectiveness records and {len(old_prefs)} preference records"
            )
