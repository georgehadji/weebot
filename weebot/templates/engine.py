...
class TemplateEngine:
    ...
    def register_task_handler(
        self, task_type: str, handler: Callable[[dict[str, Any], ExecutionContext], Any]
    ) -> None:
        """
    ...
    def execute(
        ...
        context = ExecutionContext(template=template, parameters=resolved_params)
    ...
    def _execute_workflow(
        self, template: WorkflowTemplate, context: ExecutionContext
    ) -> TemplateExecutionResult:
        """
        ...
        for task_id, task_def in template.workflow.items():
            ...
            if task_type in self._task_handlers:
                ...
                try:
                    ...
                    task_results.append({"task_id": task_id, "success": True, "result": result})
                    # Store result as variable for later tasks
                    variables[task_id] = result
                except Exception as e:
                    task_results.append({"task_id": task_id, "success": False, "error": str(e)})
    ...
    def _build_output(
        self, template: WorkflowTemplate, context: ExecutionContext, variables: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def quick_execute(
        self, template_yaml: str, parameters: dict[str, Any] | None = None
    ) -> TemplateExecutionResult:
        """
