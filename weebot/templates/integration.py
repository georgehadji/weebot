...
class TemplateOrchestratorIntegration:
    ...
    def _handle_agent_task(
        self, task_def: dict[str, Any], context: ExecutionContext
    ) -> dict[str, Any]:
        """
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as pool:
                        future = pool.submit(
                            asyncio.run, handler.handle_agent_task(task_def, context)
                        )
                        return future.result()
                else:
                    return loop.run_until_complete(handler.handle_agent_task(task_def, context))
            except RuntimeError:
    ...
    def _handle_parallel_tasks(
        ...
        if self.orchestrator and HAS_ORCHESTRATOR:
            ...
            try:
                workflow_result = asyncio.run(self.orchestrator.execute(task_graph))
    ...
    def _execute_with_orchestrator(
        ...
        try:
            ...
            return {"success": False, "error": f"Parameter resolution failed: {e}"}
            ...
            if isinstance(value, str):
                ...
                for item in value:
                    if isinstance(item, str):
                        ...
                        for param_name, param_value in parameters.items():
                            placeholder = f"{{{{{param_name}}}}}"
                            resolved_item = resolved_item.replace(placeholder, str(param_value))
...
class TemplateCLI:
    ...
    def __init__(self, integration: TemplateOrchestratorIntegration | None = None):
    ...
    def show_template(self, name: str) -> dict[str, Any] | None:
        """Show template details."""
        return self.integration.engine.registry.get_metadata(name)

    def execute(
        self, template_name: str, parameters: dict[str, Any] | None = None, dry_run: bool = False
    ) -> dict[str, Any]:
        """
        Execute a template.
        ...
        """
        if dry_run:
            return self.integration.engine.execute(template_name, parameters, dry_run=True)
        else:
            return self.integration.execute_workflow_template(template_name, parameters)

    def validate(self, template_name: str, parameters: dict[str, Any] | None = None) -> list[str]:
        """Validate template execution."""
        return self.integration.engine.validate(template_name, parameters)


def create_integrated_engine(
    load_builtin: bool = True, use_orchestrator: bool = True
    ...
    integration = TemplateOrchestratorIntegration(engine=engine, orchestrator=orchestrator)
