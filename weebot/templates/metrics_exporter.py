...
class HardenModeMetrics:
    ...
    def export_circuit_breaker_metrics(self, circuit_breaker) -> dict[str, Any]:
        ...
        return {
            ...
            "weebot_circuitbreaker_recovery_attempts_total": metrics_data.get(
                "recovery_attempts", 0
            ),
            "weebot_circuitbreaker_successful_recoveries_total": metrics_data.get(
    ...
    def export_all_metrics(
        self, audit_middleware=None, rate_limiter=None, circuit_breaker=None, pool_monitor=None
    ) -> dict[str, Any]:
        """
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["path"] == "/metrics":
            ...
            metrics = self.exporter.export_all_metrics(
                self.audit, self.rate_limiter, self.circuit_breaker, self.pool_monitor
            )
...
def create_metrics_endpoint(
    audit_middleware=None, rate_limiter=None, circuit_breaker=None, pool_monitor=None
):
    """
    ...
    metrics = exporter.export_all_metrics(
        audit_middleware, rate_limiter, circuit_breaker, pool_monitor
    )
