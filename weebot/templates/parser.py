...
class SecureYamlLoader(yaml.SafeLoader):
    ...
    def compose_node(self, parent, index):
        ...
        if self._depth > self.MAX_DEPTH:
            raise TemplateSecurityError(f"YAML nesting exceeds maximum depth of {self.MAX_DEPTH}")

        # Check node count limit
        if self._node_count > self.MAX_NODES:
            raise TemplateSecurityError(f"YAML document exceeds maximum of {self.MAX_NODES} nodes")
...
class TemplateParser:
    ...
    def _parse_parameter_schema(self, name: str, param_def: dict[str, Any]) -> ParameterSchema:
        """Parse parameter definition into ParameterSchema."""
        param_type = param_def.get("type", "string")
        if param_type not in self.SUPPORTED_TYPES:
            raise TemplateValidationError(f"Parameter '{name}' has unsupported type '{param_type}'")
