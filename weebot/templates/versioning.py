...
class TemplateVersionManager:
    ...
    def get_latest_version(self, template_name: str) -> TemplateVersion | None:
    ...
    def deprecate_version(
        self, template_name: str, version: str, message: str, replaced_by: str | None = None
    ):
        """
    ...
    def compare_versions(self, template_name: str, version1: str, version2: str) -> int:
        """
    ...
    def migrate_parameters(
        self, template_name: str, from_version: str, to_version: str, parameters: dict[str, Any]
    ) -> dict[str, Any]:
        """
            ...
            if not _unsafe_migration_scripts_enabled():
                ...
                try:
                    transformed = evaluate_migration_script(
                        migration.transformation_script, dict(migrated), dict(migrated)
                    )
                    if isinstance(transformed, dict):
                        migrated = transformed
                    else:
                        _log.error("Migration transformation returned non-dict result; ignoring.")
    ...
    def _compute_hash(self, template: WorkflowTemplate) -> str:
        ...
        content = json.dumps(
            {"name": template.name, "version": template.version, "workflow": template.workflow},
            sort_keys=True,
        )
    ...
    def _save_versions(self, template_name: str, versions: list[TemplateVersion]):
        ...
        data = {"template_name": template_name, "versions": [asdict(v) for v in versions]}

        with open(versions_file, "w") as f:
            json.dump(data, f, indent=2)

    def _save_template_snapshot(self, template_name: str, version: str, template: WorkflowTemplate):
        """Save template snapshot."""
    ...
    def _save_migration(self, template_name: str, migration: VersionMigration):
        ...
        for i, m in enumerate(migrations):
            if (
                m["from_version"] == migration.from_version
                and m["to_version"] == migration.to_version
            ):
    ...
    def _get_migration(
        self, template_name: str, from_version: str, to_version: str
...
class VersionedTemplateRegistry:
    ...
    def register_with_version(
        self, template: WorkflowTemplate, author: str, changelog: str
    ) -> TemplateVersion:
    ...
    def get_template(
        self, name: str, version: str | None = None, check_deprecated: bool = True
