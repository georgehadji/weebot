...
class HookRegistry:
    ...
    def __init__(self):
        self._hooks: dict[str, list[Hook]] = {stage: [] for stage in self.VALID_STAGES}
...
def hook(stage: str, priority: int = 0, condition: Callable | None = None):
    """
    ...
    def decorator(func: HookFunction) -> HookFunction:
        ...
        func._hook_info = {"stage": stage, "priority": priority, "condition": condition}
        return func
    ...
    def execute(self, template_name: str, parameters=None, dry_run=False):
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    future = pool.submit(
                        asyncio.run, self.hooks.execute_hooks(HookRegistry.PRE_EXECUTE, context)
                    )
            ...
            context = asyncio.run(self.hooks.execute_hooks(HookRegistry.PRE_EXECUTE, context))
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    future = pool.submit(
                        asyncio.run, self.hooks.execute_hooks(HookRegistry.POST_EXECUTE, context)
                    )
                    context = future.result()
            else:
                context = loop.run_until_complete(
            ...
            context = asyncio.run(self.hooks.execute_hooks(HookRegistry.POST_EXECUTE, context))
...
class BuiltinHooks:
    ...
    def error_handler(template_name, parameters, result, **kwargs):
        ...
        if not result.success:
            _log.error(f"Template '{template_name}' failed: {result.error}")
        return {}
...
class BuiltinPlanActHooks:
    ...
    def log_plan_created(session_id: str, step_count: int, elapsed_ms: float, **_):
        _log.info(
            "[Hook] Plan created: %d steps in %.0fms (session=%s)",
            step_count,
            elapsed_ms,
            session_id,
        )
    ...
    @staticmethod
    def log_step_start(
        session_id: str, step_id: str, step_description: str, step_index: int, total_steps: int, **_
    ):
        _log.info(
            "[Hook] Step %d/%d starting: %s (session=%s)",
            step_index + 1,
            total_steps,
            step_description[:80],
            session_id,
    ...
    def log_step_done(session_id: str, step_id: str, elapsed_ms: float, **_):
        _log.info("[Hook] Step %s done in %.0fms (session=%s)", step_id, elapsed_ms, session_id)

    @staticmethod
    def log_verification(session_id: str, scores: dict, gate_failures: list, **_):
        _log.info(
            "[Hook] Verification: scores=%s gates_failed=%s (session=%s)",
            scores,
            gate_failures,
            session_id,
        )
    ...
    @staticmethod
    def log_complete(
        session_id: str, tool_count: int, error_count: int, total_elapsed_ms: float, **_
    ):
        _log.info(
            "[Hook] Flow complete: %d tools, %d errors, %.0fms (session=%s)",
            tool_count,
            error_count,
            total_elapsed_ms,
            session_id,
    ...
    @staticmethod
    def alert_on_blocked_command(
        session_id: str, command: str, risk_level: str, allowed: bool, **_
    ):
        if not allowed:
            _log.warning(
                "[SECURITY] Blocked command '%s...' (session=%s)", command[:60], session_id
            )
    ...
    def register_all(registry: HookRegistry) -> None:
        ...
        registry.register(
            "post_plan_created",
            BuiltinPlanActHooks.log_plan_created,
            priority=0,
            name="log_plan_created",
        )
        registry.register(
            "pre_task", BuiltinPlanActHooks.log_step_start, priority=0, name="log_step_start"
        )
        registry.register(
            "post_task", BuiltinPlanActHooks.log_step_done, priority=0, name="log_step_done"
        )
        registry.register(
            "post_verification",
            BuiltinPlanActHooks.log_verification,
            priority=0,
            name="log_verification",
        )
        registry.register(
            "post_complete", BuiltinPlanActHooks.log_complete, priority=0, name="log_complete"
        )
        registry.register(
            "post_bash_guard",
            BuiltinPlanActHooks.alert_on_blocked_command,
            priority=100,
            name="alert_blocked_command",
...
def example_hooks():
    ...
    engine.hooks.register(
        stage="post_execute", function=my_custom_hook, priority=5, name="my_custom_hook"
    )
