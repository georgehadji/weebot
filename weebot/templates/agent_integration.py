...
class TemplateAgentManager:
    ...
    def get_or_create_agent(
        self, role: str, task_description: str | None = None
    ...
    def _create_agent_for_role(
        ...
        _log.debug(
            "Agent construction deferred (simulation mode): role=%s config=%s",
            role,
            config.project_id,
        )
        return agent
    ...
    async def execute_task(
        self, role: str, task: str, context: ExecutionContext | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": True, "agent_role": role, "task": task, "status": status}
        except Exception as e:
            _log.exception(f"Agent task execution failed: {e}")
            return {"success": False, "agent_role": role, "task": task, "error": str(e)}
...
class TemplateAgentTaskHandler:
    ...
    def __init__(self, agent_manager: TemplateAgentManager | None = None):
    ...
    async def handle_agent_task(
        ...
        return await self.agent_manager.execute_task(
            role=agent_role, task=task_description, context=context
        )

    def _simulate_execution(self, role: str, task: str) -> dict[str, Any]:
...
def register_agent_handlers(engine, agent_manager: TemplateAgentManager | None = None) -> None:
    """
...
def create_agent_enabled_engine(
    load_builtin: bool = True, agent_manager: TemplateAgentManager | None = None
):
    """
