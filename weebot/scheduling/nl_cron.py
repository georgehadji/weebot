...
def parse_schedule(text: str) -> dict | None:
    ...
    if "every hour" in text_lower or re.search(r"\bevery\s+hour\b", text_lower):
        return {"cron_expression": "0 * * * *", "description": "Every hour"}
    ...
    if "every day" in text_lower or "daily" in text_lower:
        ...
        return {"cron_expression": f"{m} {h} * * *", "description": f"Daily at {h:02d}:{m:02d}"}
    ...
    day_match = re.search(rf"\b(({day_names})(\s+and\s+({day_names}))?)\b", text_lower)
    if day_match:
        h = hour if hour is not None else 9
        m = minute if minute is not None else 0
        days = []
