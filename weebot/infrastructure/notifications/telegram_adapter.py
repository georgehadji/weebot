...
class TelegramAdapter(NotificationPort):
    ...
    def _format_message(self, notification: Notification) -> str:
        ...
        if notification.metadata:
            ...
            for key, value in notification.metadata.items():
                lines.append(
                    f"• {self._escape_markdown(key)}: `{self._escape_markdown(str(value))}`"
                )
    ...
    async def notify(self, notification: Notification) -> NotificationResult:
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                ...
                if response.status == 200 and result.get("ok"):
                    ...
                    return NotificationResult(
                        ...
                        channel=self.channel,
                        metadata={"telegram_message_id": message_id, "chat_id": chat_id},
                    )
                else:
            ...
            return NotificationResult(
                success=False, notification_id=notification.id, channel=self.channel, error=str(e)
            )
