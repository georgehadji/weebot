...
class SandboxBackendAdapter(BackendPort):
    ...
    def __init__(self, sandbox: SandboxPort | None = None, default_timeout: int = 60) -> None:
        if sandbox is None:
            raise ValueError(
                "SandboxBackendAdapter requires a SandboxPort. " "Inject via __init__(sandbox=...)."
            )
        self._sandbox = sandbox
        self._default_timeout = default_timeout

    async def _shell(self, script: str, timeout: int | None = None) -> str:
        """Run a PowerShell script and return combined output."""
        result = await self._sandbox.execute_shell(
            script=script, shell="powershell", timeout=float(timeout or self._default_timeout)
        )
        return result.combined_output or ""

    async def _shell_json(self, script: str, timeout: int | None = None) -> dict | list:
    ...
    async def read(self, file_path: str, offset: int = 0, limit: int = 100) -> ReadResult:
        try:
            ...
            total_data = await self._shell_json(f"(Get-Content -Path '{escaped}').Count")
    ...
    async def grep(
        ...
        try:
            ...
            cmd = "Get-ChildItem -Recurse -File"
            if path:
                escaped_path = path.replace("'", "''")
    ...
    async def execute(self, command: str, timeout: int | None = None) -> ExecuteResult:
        try:
            result = await self._sandbox.execute_shell(
                script=command, shell="powershell", timeout=float(timeout or self._default_timeout)
            )
