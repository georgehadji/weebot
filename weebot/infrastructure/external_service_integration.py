...
class ExternalService(ABC):
    ...
    def __init__(self, config: ServiceConfig, circuit_breaker: CircuitBreaker | None = None):
        self.config = config
...
class APIService(ExternalService):
    ...
    async def execute(self, operation: str, **kwargs) -> ServiceResponse:
        ...
        if not self.config.enabled:
            return ServiceResponse(success=False, error="Service is disabled", status_code=503)
...
class ServiceRegistry:
    ...
    async def get_service(self, name: str) -> ExternalService | None:
        """Get a registered service by name."""
        return self.services.get(name)

    async def execute_service_operation(
        self, service_name: str, operation: str, **kwargs
    ) -> ServiceResponse:
        """Execute an operation on a registered service."""
        service = await self.get_service(service_name)
        if not service:
            return ServiceResponse(
                success=False, error=f"Service '{service_name}' not found", status_code=404
            )
    ...
    async def call_external_service(
        ...
        try:
            ...
            if response.success:
                ...
                return ToolResult(content=f"Service call failed: {response.error}", tool_error=True)
        except Exception as e:
            self.logger.error(f"Error calling external service {service_name}: {e}")
            return ToolResult(content=f"Error calling external service: {str(e)}", tool_error=True)
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        response = await registry.execute_service_operation(
            "test_api", "get", method="GET", params={"key": "value"}
        )
