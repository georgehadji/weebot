...
class WSL2Sandbox(SandboxPort):
    ...
    def __init__(
        self, config: SandboxConfig | None = None, distribution: str | None = None
    ) -> None:
    ...
    async def is_available(self) -> bool:
        ...
        try:
            ...
            proc = await asyncio.create_subprocess_exec(
                "wsl", "--status", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
    ...
    async def list_distributions(self) -> list[str]:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                "wsl", "-l", "-q", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
