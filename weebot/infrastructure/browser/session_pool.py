...
class BrowserSessionPool:
    ...
    @asynccontextmanager
    async def acquire(self, config: BrowserConfig | None = None, timeout: float = 30.0):
        """
        ...
        async with self._semaphore:
            try:
                ...
                session = await asyncio.wait_for(
                    self._available.get(), timeout=1.0  # Short timeout for available sessions
                )

                # Check if expired or config mismatch
                if session.is_expired(self.session_ttl) or not self._config_matches(
    ...
    async def _create_session(self, config: BrowserConfig | None = None) -> BrowserSession:
    ...
    def _config_matches(self, cached: BrowserConfig, requested: BrowserConfig) -> bool:
        ...
        return (
            cached.browser_type == requested.browser_type
            and cached.headless == requested.headless
            ...
            try:
                ...
                while not self._available.empty():
                    try:
                        ...
                        if session.is_expired(self.session_ttl) or session.is_idle(
                            self.idle_timeout
                        ):
                            await session.close()
                            self._stats["expired"] += 1
...
async def get_browser_pool(
    min_sessions: int = 1, max_sessions: int = 5, **kwargs
) -> BrowserSessionPool:
    ...
    if not _DI_WARNED:
        logger.warning("get_browser_pool() is deprecated. " "Use container.get('browser_pool').")
        _DI_WARNED = True

    async with _pool_lock:
        if _global_pool is None:
            _global_pool = BrowserSessionPool(
                min_sessions=min_sessions, max_sessions=max_sessions, **kwargs
            )
...
async def close_global_pool() -> None:
    ...
    if not _DI_WARNED:
        logger.warning(
            "close_global_pool() is deprecated. " "Use container.get('browser_pool').close()."
        )
        _DI_WARNED = True
