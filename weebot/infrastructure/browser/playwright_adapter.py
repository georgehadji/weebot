...
class PlaywrightAdapter(BrowserPort):
    ...
    async def navigate(self, url: str, wait_until: str = "networkidle") -> NavigationResult:
        ...
        try:
            ...
            if response:
                ...
                return NavigationResult(
                    success=True, url=page.url, status_code=0, error=None, load_time_ms=elapsed_ms
                )
        except Exception as e:
            elapsed_ms = (time.monotonic() - t_start) * 1000
            return NavigationResult(
                success=False, url="", status_code=0, error=str(e), load_time_ms=elapsed_ms
            )
    ...
    async def get_state(self) -> BrowserState:
        ...
        async with self._lock:
            page = self._page
            if not page:
                return BrowserState(url="", title="", ready_state="", viewport=(0, 0))
    ...
    async def wait_for_selector(
        self, selector: str, timeout: float = 30.0, state: str = "visible"
    ) -> bool:
