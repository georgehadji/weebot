...
class NotificationResult:
    ...
    def __post_init__(self):
        if self.delivered_at is None:
            object.__setattr__(self, "delivered_at", datetime.now())
...
class NotificationPort(ABC):
    ...
    async def notify_many(self, notifications: list[Notification]) -> list[NotificationResult]:
...
class NotificationBus:
    ...
    async def publish(self, notification: Notification) -> None:
        ...
        await asyncio.gather(
            *[s.on_notification(notification) for s in self._subscribers], return_exceptions=True
        )
