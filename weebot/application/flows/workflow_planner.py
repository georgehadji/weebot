...
class WorkflowPlanner:
    ...
    def create_workflow_plan(
        self, user_requirement: str, intent_result: IntentRecognitionResult
    ) -> WorkflowPlan | None:
        """
    ...
    def _infer_category_from_keywords(self, user_requirement: str) -> TaskCategory | None:
        ...
        if any(
            ...
            for keyword in ["analyze", "data", "statistics", "report", "metrics"]
        ):
            return TaskCategory.DATA_ANALYSIS
        elif any(
            keyword in text_lower
            for keyword in ["write", "create", "draft", "blog", "article", "document"]
        ):
            return TaskCategory.CONTENT_CREATION
        elif any(
            keyword in text_lower for keyword in ["automate", "schedule", "workflow", "process"]
        ):
            return TaskCategory.SYSTEM_AUTOMATION
        elif any(
            keyword in text_lower
            for keyword in ["research", "find", "study", "investigate", "explore"]
        ):
    ...
    def _generate_tasks_for_category(
        self, category: TaskCategory, user_requirement: str, intent_result: IntentRecognitionResult
    ) -> list[PlannedTask]:
    ...
    def _create_task_parameters(
        self, task_name: str, intent_result: IntentRecognitionResult
    ) -> dict[str, Any]:
    ...
    def validate_plan(self, plan: WorkflowPlan) -> tuple[bool, list[str]]:
        ...
        try:
            graph = DependencyGraph(
                {task.id: {"deps": plan.dependencies[task.id]} for task in plan.tasks}
            )
            graph.validate()
...
if __name__ == "__main__":
    ...
    for request in test_requests:
        ...
        if plan:
            ...
            for task in plan.tasks:
                print(
                    f"  - {task.name}: {task.description} ({task.estimated_duration_minutes} min)"
                )
