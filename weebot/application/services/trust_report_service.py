...
class TrustReportService(TrustReportPort):
    ...
    async def compute(
        self, session_id: str, plan_steps: list[Any], session_events: list[Any]
    ) -> TrustReport:
        try:
            ...
            return TrustReport(
                ...
                contributing_factors=[
                    ...
                    *(["CoVe inconsistency detected"] if drift > 0 else []),
                ],
            )
        except Exception as exc:
            logger.warning("TrustReport computation failed: %s", exc)
            return TrustReport(session_id=session_id, trust_band=TrustBand.CLEAN)
