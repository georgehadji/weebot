...
class StrategyAdapter:
    ...
    def __init__(self):
        ...
        self.current_strategies: dict[StrategyType, dict[str, Any]] = {
            ...
            StrategyType.AGENT_SELECTION: {
                "selection_method": "capability_match",
                "fallback_strategy": "generalist",
            },
            StrategyType.WORKFLOW_STRUCTURE: {
                "optimization_level": "moderate",
                "parallelization": True,
            },
            StrategyType.TOOL_CHOICE: {
                "preference_weight": "reliability",
    ...
    def record_performance_metric(self, metric: PerformanceMetric):
        ...
        for rec in recommendations:
            ...
            self.logger.info(
                f"Adaptation recommended: {rec.action} for {rec.strategy_type.value} - {rec.reason}"
            )
    ...
    def apply_adaptation(self, recommendation: AdaptationRecommendation) -> bool:
        ...
        try:
            ...
            if recommendation.strategy_type in self.current_strategies:
                self.current_strategies[recommendation.strategy_type].update(
                    recommendation.parameters
                )

                self.logger.info(
                    f"Applied adaptation: {recommendation.action} for {recommendation.strategy_type.value}"
                )
                return True
            else:
                self.logger.warning(
                    f"No current strategy found for type: {recommendation.strategy_type.value}"
                )
                return False
    ...
    def _analyze_performance_and_recommend_adaptations(
        ...
        if strategy_types:
            filtered_metrics = [
                m for m in metrics if any(st.value in m.name for st in strategy_types)
            ]
        else:
    ...
    def _recommend_adaptation_for_metric(
        self, metric: PerformanceMetric, trigger: AdaptationTrigger | None
    ) -> AdaptationRecommendation | None:
        ...
        if "success_rate" in metric.name:
            return AdaptationRecommendation(
                ...
                parameters={
                    "retry_attempts": min(
                        10,
                        self.current_strategies[StrategyType.ERROR_HANDLING].get(
                            "retry_attempts", 3
                        )
                        + 1,
                    )
            ...
            return AdaptationRecommendation(
                ...
                parameters={
                    "cpu_quota": min(
                        1.0,
                        self.current_strategies[StrategyType.RESOURCE_ALLOCATION].get(
                            "cpu_quota", 0.8
                        )
                        + 0.1,
                    )
            ...
            return AdaptationRecommendation(
                ...
                parameters={
                    "memory_limit_mb": self.current_strategies[
                        StrategyType.RESOURCE_ALLOCATION
                    ].get("memory_limit_mb", 1024)
                    * 1.2
                },
                confidence=0.75,
    ...
    def _analyze_patterns_and_recommend_adaptations(
        self, trigger: AdaptationTrigger | None
    ) -> list[AdaptationRecommendation]:
    ...
    def adapt_workflow_plan(
        self, plan: WorkflowPlan, intent_result: IntentRecognitionResult | None = None
    ) -> WorkflowPlan:
        """
    ...
    def _apply_structural_adaptations(
        self, plan: WorkflowPlan, intent_result: IntentRecognitionResult | None
    ) -> WorkflowPlan:
    ...
    def get_current_strategy_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_adaptations": (
                len(self.adaptation_history[-10:]) if self.adaptation_history else 0
            ),
            "performance_metrics_count": len(self.performance_history),
            "current_strategies": {k.value: v for k, v in self.current_strategies.items()},
        }
...
if __name__ == "__main__":
    ...
    adapter.record_performance_metric(
        PerformanceMetric(
            name="success_rate",
            value=0.75,
            threshold=0.8,
