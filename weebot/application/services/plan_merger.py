...
class PlanMerger:
    ...
    async def merge(self, candidates: list[PlanCandidate], original_prompt: str) -> Plan | None:
    ...
    async def _select_best(
        self, candidates: list[PlanCandidate], original_prompt: str
    ) -> PlanCandidate | None:
        """LLM call: evaluate and rank plan candidates."""
        candidates_text = ""
        for i, c in enumerate(candidates):
            steps_text = "\n".join(f"    {s.id}. {s.description}" for s in c.steps)
            candidates_text += f"Candidate {i + 1} (score={c.score:.2f}):\n{steps_text}\n\n"
        ...
        try:
            response = await self._llm.chat(
                messages=[{"role": "user", "content": sys_prompt}], max_tokens=200
            )
            ...
            if 0 <= best_idx < len(candidates):
                ...
                logger.info(
                    "DPPM merger selected candidate %d: %s", best_idx + 1, data.get("reason", "")
                )
                return selected
