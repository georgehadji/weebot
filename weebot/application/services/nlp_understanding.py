...
class IntentRecognitionResult:
    ...
    purpose_analysis: PurposeAnalysis | None = None  # Detailed purpose analysis
...
class IntentRecognizer:
    ...
    def __init__(self):
        ...
        self.entity_patterns = {
            ...
            "timeframe": [
                r"(?:in|during|over)\s+(?:the\s+)?(\w+\s+\w+|\w+)",
                r"by\s+(\w+\s+\w+|\w+)",
            ],
...
class NaturalLanguageProcessor:
    ...
    def _update_context_with_interaction(self, context: ConversationContext, user_input: str):
        ...
        if (
            len(context.previous_topics) == 0
            or context.previous_topics[-1] != context.current_topic
        ):
    ...
    def get_conversation_context(self, user_id: str) -> ConversationContext | None:
...
if __name__ == "__main__":
    ...
    for inp in conversation_inputs:
        ...
        print(f"Intent: {result.intent.value}")
        print(
            f"Resolved input: {processor._resolve_references(inp, processor._get_or_create_context(user_id))}"
        )
