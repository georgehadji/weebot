...
def _truncate_with_head_tail(
    text: str, head_chars: int, tail_chars: int, elision: str = " [...] "
) -> str:
...
class LossyContextCompressor(IContextEnginePort):
    ...
    async def should_compress(
        self, messages: list[dict[str, Any]], token_count: int, threshold: int = 12000
    ) -> bool:
    ...
    async def compress(
        ...
        compressible = [m for m in messages if m not in system_msgs and m not in volatile_msgs]
        ...
        compressed_messages = (
            system_msgs + [{"role": "system", "content": summary_text}] + volatile_msgs
        )
