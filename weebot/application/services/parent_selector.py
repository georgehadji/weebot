...
class ParentSelector:
    ...
    def __init__(self, store: SkillVariantStorePort, top_k: int = 3) -> None:
        self._store = store
        self._top_k = top_k

    async def select(
        self, domain: str, top_k: int | None = None, min_score: float = 0.0
    ) -> list[SkillVariant]:
        ...
        filtered: list[SkillVariant] = [v for v in candidates if v.score >= min_score]
