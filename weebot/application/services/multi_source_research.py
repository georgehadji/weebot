                ...
                if response.status == 200:
                    ...
                    async with self.session.get(search_url, params=params) as search_response:
                        if search_response.status == 200:
                            ...
                            self.logger.warning(
                                f"Wikipedia search failed with status {search_response.status}"
                            )
                            return []
                else:
...
class MultiSourceResearchEngine:
    ...
    def __init__(
        ...
        browser_tool: AdvancedBrowserTool | None = None,
    ...
    async def remove_provider(self, provider_type: ResearchSourceType):
        ...
        self.providers = [
            p for p in self.providers if p.get_source_info().source_type != provider_type
        ]
    ...
    async def conduct_research(
        ...
        if self._rerank is not None and all_results:
            try:
                ...
                documents = [
                    f"{r.get('title', '')}: {r.get('snippet', '')[:500]}" for r in all_results
                ]
                ...
                sorted_results = [
                    all_results[rr.index] for rr in reranked if rr.index < len(all_results)
                ]
                self.logger.debug(
                ...
                self.logger.warning(
                    "Research rerank failed, falling back to metadata sort: %s", exc
                )
    ...
    async def _generate_summary(self, query: str, results: list[dict[str, Any]]) -> str:
        ...
        summary = (
            f"Found {len(results)} results for '{query}'. Top sources include: {', '.join(titles)}"
        )
...
class ResearchTool:
    ...
    async def conduct_research(
        self, query: str, max_sources: int = 5, max_results_per_source: int = 5
    ) -> ToolResult:
        """Conduct research and return results."""
        try:
            result = await self.research_engine.conduct_research(
                query=query, max_sources=max_sources, max_results_per_source=max_results_per_source
            )
            ...
            return ToolResult(
                content=json.dumps(formatted_result, indent=2, default=str), tool_error=False
            )
        except Exception as e:
            self.logger.error(f"Error conducting research: {e}")
            return ToolResult(content=f"Error conducting research: {str(e)}", tool_error=True)
