...
class OptimizerAgent(OptimizerPort):
    ...
    def __init__(self, optimizer_llm: LLMPort, event_bus: EventBusPort | None = None):
    ...
    async def reflect_on_evaluator(
        ...
        prompt = self._evaluator_reflection_prompt().format(
            current_prompt=current_evaluator.prompt, trajectory_summary=trajectory_summary
        )
    ...
    async def merge_edits(
        ...
        merged_failures = await self._merge_group(merge_failures_prompt, failure_edits)

        # Stage 2: consolidate success proposals
        merged_successes = await self._merge_group(merge_successes_prompt, success_edits)

        # Stage 3: combine with failure priority
        return await self._merge_final(merge_final_prompt, merged_failures, merged_successes)
    ...
    async def rank_edits(
        ...
        response = await self._llm.chat(
            messages=[
                {
                    "role": "system",
                    "content": prompt.replace("{budget}", str(budget)).replace(
                        "{skill_content}", current_skill.content[:2000]
                    ),
                },
                {"role": "user", "content": json.dumps(edits_json, indent=2)},
            ],
            response_format={"type": "json_object"},
    ...
    async def slow_update(
        ...
        return await self._produce_guidance(prompt, prev_skill, curr_skill, longitudinal_data)
    ...
    @staticmethod
    def _partition(
        items: list[TrajectorySummary], minibatch_size: int
    ...
    async def _reflect_minibatch(
        ...
        trajectories_json = json.dumps([t.model_dump() for t in minibatch], indent=2)
    ...
    async def _merge_final(
        self, system_prompt: str, failure_edits: list[SkillEdit], success_edits: list[SkillEdit]
    ) -> list[SkillEdit]:
