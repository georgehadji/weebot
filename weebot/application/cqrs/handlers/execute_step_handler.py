...
class ExecuteStepHandler(CommandHandler):
    ...
    async def handle(self, command: ExecuteStepCommand) -> CommandResult:
        ...
        try:
            ...
            if step is None:
                return CommandResult.fail(
                    error=f"Step {command.step_id} not found in plan", error_code="STEP_NOT_FOUND"
                )
            ...
            async for event in executor.execute_step(
                plan, step, user_input=command.user_input, session_id=command.session_id
            ):
            ...
            _logging.getLogger(__name__).error("ExecuteStepHandler failed: %s\n%s", exc, detail)
            return CommandResult.fail(
