...
class ExtractFailureSignatureCommand(Command):
    ...
    failure_modes: list[str] = Field(
        default_factory=list, description="Existing failure mode labels (if any)"
    )
...
class BatchExtractSignaturesCommand(Command):
    ...
    force_reprocess: bool = Field(
        default=False, description="Re-extract even for sessions that already have signatures"
    )
