...
class BashTool(BaseTool):
        ...
        if sandbox is None:
            ...
            try:
                ...
                _logging.getLogger(__name__).warning(
                    "BashTool: no SandboxPort injected and default sandbox "
                    "creation failed (%s) — execution will error until a "
                    "sandbox is set",
                    exc,
                )
    ...
    def _legacy_validate_no_encoded_commands(self, command: str) -> tuple[bool, str]:
        ...
        for pattern in self._ENCODED_COMMAND_PATTERNS:
            if re.search(pattern, command, re.IGNORECASE):
                return False, (
                    "Security Error: Command contains encoded/obfuscated content "
    ...
    async def _validate_security(
        self, command: str, security_override: str | None = None
    ) -> tuple[bool, str]:
        """
        ...
        if self._security_enabled and self._security_analyzer:
            try:
                ...
                if assessment.risk_level == CommandRiskLevel.DANGEROUS:
                    ...
                    if security_override and self._verify_override_token(
                        command, security_override
                    ):
                        import logging
    ...
    async def execute(
        ...
        **kwargs: object,
        ...
        try:
            effective_timeout = (
                float(timeout) if timeout is not None else float(self._default_timeout)
            )
        ...
        if _bash_guard_hooks is not None:
            ...
            try:
                ...
                _loop.create_task(
                    _bash_guard_hooks.execute_hooks(
                        "post_bash_guard",
                        {
        ...
        if approval.requires_confirmation:
            return ToolResult(
                output="",
                error=(f"Command blocked — requires user confirmation: {approval.undo_hint}"),
            )
        ...
        result = await self._sandbox.execute_shell(
            script=command, shell=shell_type, timeout=effective_timeout, cwd=working_dir
        )
