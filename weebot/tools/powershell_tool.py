...
class PowerShellTool(BaseTool):
    ...
    def __init__(self, sandbox: SandboxPort | None = None, tool_config: ToolConfig | None = None):
        ...
        if sandbox is None:
            ...
            try:
                ...
                _logging.getLogger(__name__).warning(
                    "PowerShellTool: no SandboxPort injected and default "
                    "sandbox creation failed (%s) — execution will error "
            ...
            if len(match) > 100:
                ...
                try:
                    ...
                    if any(
                        ...
                        for keyword in ["powershell", "invoke", "iex", "cmdlet"]
                    ):
                        return False, (
    ...
    async def execute(self, command: str, timeout: float | None = None, **_) -> ToolResult:  # type: ignore[override]
        ...
        if approval.requires_confirmation:
            return ToolResult(
                output="", error=f"Command requires confirmation: {approval.undo_hint}"
            )

        # Coerce and clamp timeout
        try:
            effective_timeout = (
                min(float(timeout), self._get_max_timeout()) if timeout is not None else 30.0
            )
        ...
        try:
            ...
            if s_result.timed_out:
                return ToolResult(
                    output="", error=f"Command timed out after {effective_timeout:.0f}s"
                )
