...
class IdeaContract(BaseModel):
    ...
    heat_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="urgency × novelty × confidence"
    )
    estimated_effort: str = Field(default="medium", description="'low' | 'medium' | 'high'")
    dreamer_session_id: str = Field(default="")
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    intent_verdict: str | None = Field(default=None)  # set by IntentReviewService
    main_verdict: str | None = Field(default=None)  # set by MainReviewService
