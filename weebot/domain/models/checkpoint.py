...
class FlowCheckpoint(BaseModel):
    ...
    flow_type: str = Field(
        default="PlanActFlow", description="Fully-qualified flow class name for reconstruction."
    )
    current_state: str = Field(
        default="planning",
        description="Name of the current flow state (planning|executing|updating|summarizing|completed).",
    )
    plan_snapshot: Plan = Field(description="Frozen copy of the plan with current step statuses.")
    completed_steps: list[StepCheckpoint] = Field(
        default_factory=list, description="Steps that have already completed, with their results."
    )
    conversation_summary: str = Field(
        default="", description="Compacted context from MemoryCompactor so the flow can resume."
    )
