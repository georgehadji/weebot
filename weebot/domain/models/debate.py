...
class DebateResult(BaseModel):
    ...
    consensus: list[str] = Field(default_factory=list, description="Points all viewpoints agree on")
    dissent: list[dict] = Field(
        default_factory=list, description="[{'topic': ..., 'optimist': ..., 'pessimist': ...}]"
    )
    blind_spots: list[str] = Field(
        default_factory=list, description="Important angles no viewpoint covered"
    )
