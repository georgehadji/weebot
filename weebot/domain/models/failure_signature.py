...
class FailureSignature(BaseModel):
    ...
    terminal_cause: str = Field(description="What the verifier/evaluator rejected")
    agent_behavior: str = Field(description="What the agent did that led to the cause")
    mechanism: str = Field(description="Abstract reusable failure pattern")
    trajectory_health: TrajectoryHealth | None = Field(
        default=None, description="Per-trajectory health classification (if available)"
    )
    ...
    model_id: str = Field(
        default="", description="Model that was executing when the failure occurred"
    )
...
class FailureCluster(BaseModel):
    ...
    signature: FailureSignature = Field(description="Representative signature for this cluster")
    support: int = Field(ge=1, description="Number of sessions sharing this signature")
    ...
    mean_actionability: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Mean actionability across cluster members"
    )

    @classmethod
    def from_signatures(cls, signatures: list[FailureSignature]) -> FailureCluster:
        ...
        return cls(
            ...
            mean_actionability=sum(s.actionability_score for s in signatures) / len(signatures),
        )
...
class EvidenceBundle(BaseModel):
    ...
    clusters: list[FailureCluster] = Field(
        default_factory=list, description="Failure clusters ordered by priority (descending)"
    )
    total_failures: int = Field(default=0, ge=0, description="Total failed trajectories examined")
    ...
    def top_clusters(self, n: int = 5) -> list[FailureCluster]:
        ...
        scored = sorted(self.clusters, key=lambda c: c.support * c.mean_actionability, reverse=True)
        return scored[:n]
