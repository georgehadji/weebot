...
class Step(BaseModel):
    ...
    result: str | None = Field(default=None, description="Summary of execution result")
    ...
    def mark_completed(
        self, result: str | None = None, evidence_refs: list[str] | None = None
    ) -> Step:
    ...
    def mark_failed(self, result: str | None = None) -> Step:
        updates: dict = {"status": StepStatus.FAILED}
        if result is not None:
            updates["result"] = result
        return self.model_copy(update=updates)

    def mark_unverified(self, reason: str, evidence_refs: list[str] | None = None) -> Step:
    ...
    def is_blocked(self, predecessor_sibling: Step | None = None) -> bool:
...
class PlanCritique(BaseModel):
    ...
    overall_confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall confidence that the plan will succeed"
    )
    verdict: str = Field(default="approved", description="'approved' | 'revise' | 'reject'")
...
class Plan(BaseModel):
    ...
    def update_step_status(
        self, step_id: str, status: StepStatus, result: str | None = None
    ) -> Plan:
