...
class MisalignmentEntry(BaseModel):
    ...
    symptom: str = Field(
        default="", description="'constraint_violation' | 'user_correction' | 'scope_overreach'"
    )
    constraint_text: str | None = Field(
        default=None, description="The violated constraint, if known"
    )
    step_description: str | None = Field(
        default=None, description="The step that triggered the issue"
    )
    correction_text: str | None = Field(
        default=None, description="What the user said to correct it"
    )
