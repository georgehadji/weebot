...
class OpportunityProposal(BaseModel):
    ...
    prompt: str = Field(
        default="", description="The task prompt, e.g. 'Research competitor X's new pricing model'"
    )
    source: str = Field(
    ...
    evidence: list[str] = Field(
        default_factory=list, description="Citations from KG nodes or FTS5 search results"
    )
    ...
    estimated_effort: str = Field(
        default="medium", description="'low' | 'medium' | 'high' — estimated time to complete"
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    presented: bool = Field(
        default=False, description="Whether this proposal has been shown to the user"
    )
    accepted: bool = Field(default=False, description="Whether the user accepted this proposal")
