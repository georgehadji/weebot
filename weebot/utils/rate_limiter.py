...
class RateLimitExceeded(Exception):
    ...
    def __init__(self, tool_name: str, retry_after: float):
        ...
        super().__init__(
            f"Rate limit exceeded for '{tool_name}'. " f"Retry after {retry_after:.1f} seconds."
        )


def rate_limited(tool_name: str, tokens: float = 1.0, wait: bool = False, max_wait: float = 60.0):
    """
    ...
    with _buckets_lock:
        ...
        for name, bucket in _rate_limit_buckets.items():
            status[name] = {
                ...
                "utilization": (
                    1.0 - (bucket._tokens / bucket.capacity) if bucket.capacity > 0 else 0.0
                ),
            }
        return status
