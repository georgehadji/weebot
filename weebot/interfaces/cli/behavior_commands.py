...
from weebot.core.behavior_tracker import WEEBOT_DIR, create_tracker, stop_all_trackers, LEDGER_DIR
...
def watch(directory: str, session_id: str, background: bool):
    ...
    if background:
        ...
        cmd = [
            sys.executable,
            "-m",
            "weebot.interfaces.cli.behavior_commands",
            "watch",
            str(watch_path),
            "--session-id",
            session_id,
    ...
    # Interactive mode
    def on_event(event):
        color = {"created": "green", "modified": "yellow", "deleted": "red", "moved": "cyan"}.get(
            event.event_type, "white"
        )

        click.secho(f"[{event.timestamp[11:19]}] {event.event_type:10} {event.path}", fg=color)

    tracker = create_tracker(session_id, watch_path, on_event)

    click.secho("Starting behavior tracker...", fg="blue")
...
def trust():
    ...
    color = (
        "green"
        if trust_data["score_percentage"] >= 90
        else "yellow" if trust_data["score_percentage"] >= 70 else "red"
    )
    ...
    click.echo(
        f"Last Updated: {trust_data['last_updated'][:19] if trust_data['last_updated'] else 'Never'}"
    )
    click.echo()
...
def log(count: int, session: str | None):
    """Show recent actions."""
    reporter = BehaviorReporter()
...
def override(timestamp: str, reason: str):
    ...
    if trust.mark_override(timestamp, reason):
        click.secho("Marked action as override", fg="yellow")
