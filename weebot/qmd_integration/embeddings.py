...
class LocalEmbeddings:
    ...
    async def embed_query(self, query: str) -> EmbeddingResult:
        ...
        if self._model is not None:
            ...
            return EmbeddingResult(
                embedding=embedding, model=self._model_name, dimensions=len(embedding)
            )
        elif self._fallback_model is not None:
            # Use sentence-transformers
            embedding = self._fallback_model.encode(formatted).tolist()
            return EmbeddingResult(
                embedding=embedding, model="all-MiniLM-L6-v2", dimensions=len(embedding)
            )

        raise RuntimeError("No model loaded")

    async def embed_documents(
        self, documents: list[str], titles: list[str] | None = None
    ...
    async def embed_batch(
        self, texts: list[str], is_queries: bool = False
    ) -> list[EmbeddingResult]:
        """
