...
Respond ONLY with the expanded query, no explanations."""
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            self._model = Llama(
                model_path=full_path, n_ctx=self._n_ctx, n_threads=self._n_threads, verbose=False
            )
    ...
    async def _expand_with_llm(self, query: str) -> ExpandedQuery:
        ...
        return ExpandedQuery(
            original=query, expanded=expanded, terms=terms, synonyms=synonyms, confidence=0.8
        )
    ...
    def _expand_with_rules(self, query: str) -> ExpandedQuery:
        ...
        return ExpandedQuery(
            original=query, expanded=expanded, terms=terms, synonyms=synonyms, confidence=0.6
        )
    ...
    async def expand_multiple(self, queries: list[str]) -> list[ExpandedQuery]:
        """
