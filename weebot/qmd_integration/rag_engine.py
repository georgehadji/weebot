...
class RAGEngine:
    ...
    def __init__(
        ...
        mcp_client: QMDMCPClient | None = None,
    ...
    async def retrieve(
        ...
        if use_hybrid:
            ...
            documents = await self.mcp_client.search(
                query=query, collection=collection, n=10, min_score=min_score
            )
        else:
            # Use BM25 only
            documents = await self.mcp_client.search_bm25(query=query, collection=collection, n=10)

        if not documents:
            return RAGContext(query=query, chunks=[], total_tokens=0, sources=[])

        # Get full content for top documents
        doc_ids = [doc.docid for doc in documents[:5]]
        full_docs = await self.mcp_client.multi_get(docids=doc_ids, max_lines=200)
        ...
        for doc in full_docs:
            if doc.content:
                doc_chunks = self._chunk_document(
                    doc.content, doc.file, doc.docid, doc.score, doc.collection
                )
        ...
        return RAGContext(
            query=query, chunks=selected_chunks, total_tokens=total_chars // 4, sources=sources
        )

    async def retrieve_with_vector(
        self, query: str, max_tokens: int = 2000, collection: str | None = None
    ) -> RAGContext:
        """
        ...
        documents = await self.mcp_client.vsearch(query=query, collection=collection, n=10)
    ...
    def format_for_prompt(self, context: RAGContext, include_citations: bool = True) -> str:
        """
    ...
    async def search_and_format(
        ...
        """
        context = await self.retrieve(query=query, max_tokens=max_tokens, collection=collection)
