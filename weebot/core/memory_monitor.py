...
class MemoryStats:
    ...
    max_mb: int
    percent: float
    system_percent: float | None = None  # System-wide memory %
    timestamp: float = 0.0
...
class MemoryAwareMixin:
    ...
    def __init__(self, max_workers: int = 10, memory_thresholds: MemoryThresholds | None = None):
    ...
    def _on_memory_event(self, level: str, stats: MemoryStats):
        ...
        logger.warning(
            f"Reduced workers: {old_workers} -> {new_workers} " f"due to memory pressure"
        )
...
async def get_memory_monitor(thresholds: MemoryThresholds | None = None, **kwargs) -> MemoryMonitor:
