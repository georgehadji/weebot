    ...
    async def _adjust(self) -> None:
        ...
        async with self._lock:
            ...
            if high_cpu or high_memory:
                # Scale down
                new_workers = max(self.min_workers, int(old_workers * self.scale_down_factor))
                ...
                # Scale up gradually
                new_workers = min(self.max_workers, old_workers + self.scale_up_increment)
                if new_workers > old_workers:
...
async def with_adaptive_limit(
    func, *args, controller: AdaptiveConcurrencyController | None = None, **kwargs
):
    """
