...
class EventBroker:
    ...
    def __init__(
        ...
        self._tag_index: defaultdict[str, deque[ContextEvent]] = defaultdict(
            lambda: deque(maxlen=max_per_tag)
        )
    ...
    async def publish(
        ...
        """
        event = ContextEvent(event_type=event_type, agent_id=agent_id, data=data or {})
    ...
    async def subscribe(
        self, event_type: str, agent_filter: str | None = None, queue_size: int = 100
    ...
    def get_event_history(self, event_type: str | None = None) -> list[ContextEvent]:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_subscriptions": {
                event_type: len(queues) for event_type, queues in self._subscriptions.items()
            },
        }
...
class AgentContext:
    ...
    state_repository: StateProvider | None = None
    ...
    @classmethod
    def create_child(
        cls, parent_context: AgentContext, parent_agent_id: str, role: str
    ) -> AgentContext:
    ...
    async def store_result(
        self, key: str, value: Any, tags: list[str] | None = None, lock_timeout: float = 10.0
    ) -> bool:
        """
        ...
        self.activity_stream.push(
            self.orchestrator_id, "context", f"{self.agent_id}: stored {key} (tags: {tags or []})"
        )
        return True
    ...
    async def get_result(self, key: str, lock_timeout: float = 5.0) -> Any | None:
        """
    ...
    async def get_sibling_output(self, agent_id: str) -> Any | None:
    ...
    async def publish_event(self, event_type: str, data: dict[str, Any] | None = None) -> bool:
        ...
        if self.event_publisher:
            delivered = await self.event_publisher.publish(event_type, self.agent_id, data)
        else:
            delivered = await self.event_broker.publish(event_type, self.agent_id, data)
        self.activity_stream.push(
            self.orchestrator_id, "event", f"{self.agent_id} published {event_type}"
        )
        return delivered

    async def subscribe_to_events(
        self, event_type: str, agent_filter: str | None = None
    ) -> AsyncIterator[ContextEvent]:
    ...
    async def checkpoint(self, message: str, requires_approval: bool = False) -> bool:
        """Record a checkpoint in the workflow."""
        self.activity_stream.push(self.orchestrator_id, "checkpoint", f"{self.agent_id}: {message}")
