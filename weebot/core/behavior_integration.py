...
class BehaviorIntegration:
    ...
    async def on_session_start(
        self, session_id: str, working_dir: Path | None = None, user_id: str = "default"
        ...
        try:
            tracker = create_tracker(session_id=session_id, watch_dir=watch_dir)
            tracker.start()
    ...
    async def on_session_end(self, session_id: str, generate_report: bool = True) -> dict | None:
    ...
    def _is_system_dir(self, path: Path) -> bool:
        ...
        for system_path in system_paths:
            if (
                path_str == system_path
                or path_str.startswith(system_path + "/")
                or path_str.startswith(system_path + "\\")
            ):
                return True
...
def start_session_tracking(
    session_id: str, working_dir: Path | None = None, user_id: str = "default"
) -> BehaviorTracker | None:
...
def stop_session_tracking(session_id: str, generate_report: bool = True) -> dict | None:
...
async def stop_session_tracking_async(session_id: str, generate_report: bool = True) -> dict | None:
