...
class Quantity:
    ...
    def __mul__(self, other: Union["Quantity", float]) -> "Quantity":
        if isinstance(other, Quantity):
            ...
            new_uncertainty = (
                ...
                if (self.uncertainty or other.uncertainty)
                else None
            )
            return Quantity(new_value, f"{self.unit}*{other.unit}", new_uncertainty)
        return Quantity(
            self.value * other, self.unit, self.uncertainty * other if self.uncertainty else None
        )
    ...
    def __add__(self, other: "Quantity") -> "Quantity":
        if self.unit != other.unit:
            raise ValueError(
                f"Cannot add quantities with different units: {self.unit} + {other.unit}"
            )
        new_value = self.value + other.value
        new_uncertainty = (
            np.sqrt((self.uncertainty or 0) ** 2 + (other.uncertainty or 0) ** 2)
            if (self.uncertainty or other.uncertainty)
            else None
        )
...
class ScientificValidator:
    ...
    def validate_dataset(self, df: pd.DataFrame, rules: dict | None = None) -> dict[str, Any]:
        """
