...
class Experiment:
    ...
    def __init__(self, config: ExperimentConfig, base_dir: Path, exp_id: str | None = None):
        self.config = config
    ...
    def record(self, operation: str, inputs: dict, outputs: dict, code: str | None = None):
        """Record an operation"""
    ...
    def export_package(self, output_path: str):
        ...
        # Create archive
        shutil.make_archive(output_path.with_suffix(""), "zip", self.work_dir)

        logger.info(f"Exported experiment to: {output_path}")
        return output_path

    def _save_manifest(self, status: str, error: str | None = None):
