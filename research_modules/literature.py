        ...
        try:
            ...
            if data["status"] == "ok":
                ...
                citation = Citation(
                    ...
                    authors=[
                        f"{a.get('family')}, {a.get('given', '')}" for a in work.get("author", [])
                    ],
    ...
    def search(self, query: str) -> list[Citation]:
        ...
        for citation in self.citations.values():
            if (
                ...
                or any(query_lower in a.lower() for a in citation.authors)
                or query_lower in (citation.abstract or "").lower()
            ):
...
class LiteratureReview:
    ...
    def generate_summary(self, topic: str, max_citations: int = 10) -> str:
        ...
        for i, citation in enumerate(relevant, 1):
            lines.extend([f"{i}. {citation.format_apa()}", ""])
