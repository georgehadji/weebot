...
pytestmark = pytest.mark.skipif(
    not os.getenv("VANE_BASE_URL"), reason="VANE_BASE_URL not set, skipping Vane integration tests."
)
...
async def test_vane_integration_academic_search(vane_base_url):
    ...
    assert "sources" in result.metadata
    assert any("arxiv.org" in s.get("url", "") for s in result.metadata["sources"]) or any(
        "ieee.org" in s.get("url", "") for s in result.metadata["sources"]
    )
