...
class TestCascadeParallelProbes:
    ...
    async def test_parallel_probes_dispatch_correct_models(
        ...
        mock_llm.chat.return_value = LLMResponse(content="response", tool_calls=None)
        resp = await executor.call_with_cascade(
            [{"role": "user", "content": "test"}], description="integration test"
        )
...
class TestCascadeSequentialFallback:
    ...
    async def test_sequential_fallback_after_parallel_exhaustion(
        ...
        resp = await executor.call_with_cascade(
            [{"role": "user", "content": "test"}], description="fail-then-fallback"
        )
        assert resp is not None
    ...
    async def test_fast_fail_reduces_remaining_timeouts(
        ...
        try:
            await executor._cascade_try_chat(
                [{"role": "user", "content": "test"}], model_id="auth/model", timeout=90.0
            )
        except ValueError:
...
class TestCascadeAllModelsTripped:
    ...
    @pytest.mark.asyncio
    async def test_all_models_tripped_error(self, executor: CascadeExecutor, mock_llm) -> None:
        ...
        with pytest.raises(AllModelsTrippedError):
            await executor.call_with_cascade(
                [{"role": "user", "content": "test"}], description="all-fail"
            )
...
class TestOnSuccessCallback:
    ...
    @pytest.mark.asyncio
    async def test_callback_invoked_on_success(self, executor: CascadeExecutor, mock_llm) -> None:
        """on_success is called after a successful cascade response."""
        callback = AsyncMock()
        executor._on_success = callback
        mock_llm.chat.return_value = LLMResponse(content="success", tool_calls=None)
        resp = await executor.call_with_cascade(
            [{"role": "user", "content": "test"}], description="callback-test"
        )
        assert resp is not None
