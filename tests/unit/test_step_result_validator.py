...
def test_identical_to_previous_fails(validator):
    ...
    r = validator.validate(result=result, step_description="Analyze data", previous_result=result)
    assert not r.passed
...
def test_quality_hint_references_step_description(validator):
    ...
    r = validator.validate(result="", step_description="Fetch API data")
...
def _assistant_message_event(message="Step completed."):
    ...
    return MessageEvent(type="message", role="assistant", message=message)
...
def test_file_creation_bypasses_empty_result(validator):
    ...
    r = validator.validate(
        result=None, step_description="Create a file", step_events=[_file_created_event()]
    )
    assert r.passed
...
def test_file_creation_updated_bypasses_empty(validator):
    ...
    r = validator.validate(
        ...
        step_events=[
            _file_created_event(tool_name="edit_file", result="Updated /tmp/conf.yaml (89 chars)")
        ],
    )
    assert r.passed
...
def test_file_creation_no_result_text_falls_through(validator):
    ...
    r = validator.validate(
        result=None, step_description="Create a file", step_events=[_file_created_event(result="")]
    )
    assert not r.passed
    assert "empty" in r.reason
...
def test_non_file_tool_does_not_bypass(validator):
    ...
    r = validator.validate(
        result=None, step_description="Run command", step_events=[_tool_event_no_creation()]
    )
    assert not r.passed
...
def test_empty_step_events_list_falls_through(validator):
    ...
    r = validator.validate(result=None, step_description="Do something", step_events=[])
    assert not r.passed
...
def test_step_events_default_none_unchanged(validator):
    ...
    r = validator.validate(
        result="The analysis found 42 patterns in the dataset.", step_description="Analyze data"
    )
    assert r.passed
