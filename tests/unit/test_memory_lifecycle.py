...
def service():
    return MemoryLifecycleService(
        hot_ttl=3600, warm_ttl=86400 * 7, cold_ttl=86400 * 30, hot_min_access=3, max_hot_entries=50
    )
...
def test_classify_hot_recent_frequent(service):
    ...
    entry = MemoryEntry(key="test", access_count=5, created_at=time.time() - 100)
...
def test_classify_hot_recent_low_access(service):
    ...
    entry = MemoryEntry(key="test", access_count=0, created_at=time.time() - 100)
    tier = service.classify(entry)
    assert tier == MemoryTier.HOT
...
def test_classify_warm_old(service):
    ...
    entry = MemoryEntry(
        key="test", access_count=0, created_at=time.time() - 7200  # 2 hours ago (past HOT_TTL)
    )
    tier = service.classify(entry)
...
def test_classify_cold_very_old(service):
    """Very old entry -> COLD."""
    entry = MemoryEntry(
        key="test", access_count=0, created_at=time.time() - 86400 * 14  # 14 days ago
    )
    tier = service.classify(entry)
...
def test_should_not_retain_cold_past_ttl(service):
    ...
    entry = MemoryEntry(
        key="stale", tier=MemoryTier.COLD, created_at=time.time() - 86400 * 60  # 60 days
    )
...
def test_demote_candidates_old_hot_to_warm(service):
    ...
    entry = MemoryEntry(
        key="old_hot", tier=MemoryTier.HOT, access_count=1, created_at=time.time() - 7200  # 2 hours
    )
...
def test_enforce_hot_capacity(service):
    ...
    entries = [
        MemoryEntry(key=f"hot_{i}", tier=MemoryTier.HOT, last_accessed=time.time() - (100 - i))
...
def test_enforce_hot_capacity_under_limit(service):
    ...
    entries = [MemoryEntry(key=f"hot_{i}", tier=MemoryTier.HOT) for i in range(30)]
    demoted = service.enforce_hot_capacity(entries)
