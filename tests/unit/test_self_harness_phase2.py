...
class TestEvidenceBundle:
    def test_top_clusters(self):
        ...
        clusters = [
            FailureCluster(
                signature=sig, support=10, representative_session_ids=[], mean_actionability=0.9
            ),
            FailureCluster(
                signature=sig, support=2, representative_session_ids=[], mean_actionability=0.3
            ),
        ]
...
class TestExtractFailureSignatureHandler:
    ...
    async def test_skip_missing_trace(self):
        ...
        cmd = ExtractFailureSignatureCommand(session_id="s1", trajectory_text="")
    ...
    async def test_extracts_and_saves(self):
        ...
        cmd = ExtractFailureSignatureCommand(
            session_id="s1",
            task_id="t1",
            trajectory_text="agent did X then Y then X again",
            failure_modes=["timeout"],
        )
        result = await handler.handle(cmd)
...
class TestTrajectoryRepositoryFailureSignatures:
    ...
    async def test_get_sessions_without_signature(self, db_path):
        ...
        t = TrajectorySummary(
            ...
            score=0.3,
            passed=False,
            trajectory_text="agent did X",
            failure_modes=["timeout"],
        )
        await repo.save(t)

        # Add a passing trajectory
        t2 = TrajectorySummary(task_id="t2", session_id="s2", score=0.9, passed=True)
        await repo.save(t2)
