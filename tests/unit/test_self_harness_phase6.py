...
class TestHarnessSafetyGate:
    def test_autonomous_surface(self):
        ...
        edit = HarnessEdit(target_surface="instructions.bootstrap", new_value="Check deps first")
    ...
    def test_gated_surface(self):
        ...
        edit = HarnessEdit(target_surface="runtime_control.max_recent_tool_errors", new_value="5")
        result = HarnessSafetyGate.check([edit])
    ...
    def test_all_autonomous_surfaces(self):
        ...
        edits = [HarnessEdit(target_surface=s, new_value="test") for s in AUTONOMOUS_SURFACES]
    ...
    def test_all_gated_surfaces(self):
        ...
        edits = [HarnessEdit(target_surface=s, new_value="test") for s in GATED_SURFACES]
        result = HarnessSafetyGate.check(edits)
        assert result.requires_approval
    ...
    def test_unknown_surface_is_gated(self):
        ...
        edit = HarnessEdit(target_surface="some.unknown.field", new_value="test")
        result = HarnessSafetyGate.check([edit])
    ...
    def test_approval_prompt_format(self):
        ...
        assert (
            "expected_effect" in result.approval_prompt.lower() or "Fewer" in result.approval_prompt
        )
...
class TestHarnessOptFlowSafetyIntegration:
    ...
    async def test_gated_edit_yields_wait_for_user_event(self):
        ...
        assert (
            "WaitForUserEvent" in event_types
