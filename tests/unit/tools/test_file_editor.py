...
async def test_str_replace_not_found_is_error(tmp_file):
    ...
    editor = StrReplaceEditorTool()
    result = await editor.execute("str_replace", str(tmp_file), old_str="NONEXISTENT", new_str="x")
    assert result.is_error
...
async def test_insert_adds_lines(tmp_file):
    ...
    editor = StrReplaceEditorTool()
    result = await editor.execute("insert", str(tmp_file), insert_line=1, new_str="inserted line")
    assert not result.is_error
