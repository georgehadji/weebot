...
class TestHarnessOptimizationTarget:
    ...
    async def test_save_and_rollback(self, tmp_path):
        ...
        target = HarnessOptimizationTarget(harness_path=output / "v0.2.0.yaml", output_dir=output)
        await target.load()
...
class TestHarnessEdit:
    ...
    def test_to_edit_dict(self):
        ...
        edit = HarnessEdit(target_surface="instructions.bootstrap", new_value="Do X first")
        d = edit.to_edit_dict()
    ...
    def test_promotion_decision(self):
        ...
        d = PromotionDecision(accepted=True, delta_in=0.1, delta_ho=0.05, reason="Good improvement")
        assert d.accepted
...
class TestHarnessOptFlow:
    ...
    async def test_no_held_in_tasks_finishes_immediately(self):
        ...
        target = HarnessOptimizationTarget("weebot/config/harness/v0.2.0.yaml")
        flow = HarnessOptFlow(
            llm=llm, target=target, trajectory_repo=repo, held_in_tasks=[], held_out_tasks=[]
        )
        events = []
    ...
    async def test_mine_failure_patterns_empty(self):
        ...
        target = HarnessOptimizationTarget("weebot/config/harness/v0.2.0.yaml")
        flow = HarnessOptFlow(
            llm=llm, target=target, trajectory_repo=repo, held_in_tasks=["test-task"]
        )
    ...
    async def test_propose_edits(self):
        ...
        flow = HarnessOptFlow(llm=llm, target=target, trajectory_repo=repo, max_proposals=1)
        ...
        bundle = EvidenceBundle(clusters=[cluster], total_failures=1, total_trajectories=10)

        edits = await flow._propose_edits(harness_content=target.content, bundle=bundle)
        assert len(edits) >= 1
...
class TestRegressionGate:
    ...
    async def test_stub_auto_accept_flag(self):
        ...
        decision = await gate.validate(
            baseline={"version": "0.2.0"}, candidate={"version": "0.2.1"}
        )
