...
class TestBashGuardAdversarial:
    ...
    @pytest.mark.parametrize(
        "payload", ["mkfs.ext4 /dev/sda1", "mkfs.ntfs /dev/sda1", "mkfs.fat -F32 /dev/sdb1"]
    )
    def test_formatting_blocked(self, payload):
        """Filesystem creation/formatting commands must be blocked."""
        risk, _ = self.guard.evaluate(payload)
        assert risk == RiskLevel.BLOCKED, f"Format command not blocked: {payload}"
    ...
    def test_credential_leak_detected(self, payload):
        ...
        risk, _ = self.guard.evaluate(payload)
        assert risk == RiskLevel.SUSPICIOUS, f"Credential leak not flagged: {payload}"

    @pytest.mark.parametrize("payload", [":(){ :|:& };:"])
    def test_fork_bomb_blocked(self, payload):
        """Fork bombs must be blocked."""
        risk, _ = self.guard.evaluate(payload)
        assert risk == RiskLevel.BLOCKED, f"Fork bomb not blocked: {payload}"
    ...
    def test_safe_commands_allowed(self, payload):
        ...
        risk, _ = self.guard.evaluate(payload)
        assert risk == RiskLevel.SAFE, f"Safe command flagged as {risk}: {payload}"
...
class TestPathValidatorAdversarial:
    ...
    def test_path_traversal_blocked(self, payload):
        ...
        assert result.result != ValidationResult.VALID, f"Path traversal not blocked: {payload}"

    @pytest.mark.parametrize("payload", ["file.txt\x00.exe", "config\x00.cfg"])
    ...
    @pytest.mark.parametrize(
        "path_str", ["normal_file.txt", "subdir/data.csv", "my-project/config.yaml"]
    )
...
class TestCommandValidatorAdversarial:
    ...
    def test_dangerous_python_blocked(self, payload):
        ...
        assert (
            result.result == ValidationResult.DANGEROUS_PATTERN
...
class TestInputSanitizerAdversarial:
    ...
    def test_html_injection_detected(self, payload):
        ...
        assert InputSanitizer.contains_html_injection(
            payload
    ...
    def test_sanitize_api_key_masks_correctly(self):
        ...
        assert "sk-a" in result and "pqr" in result, f"API key not masked correctly: {result}"
