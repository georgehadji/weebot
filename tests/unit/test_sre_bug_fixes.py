...
class TestBug001ValidationRunnerBootstrap:
    ...
    def runner(self):
        ...
        return ValidationRunner(task_runner=task_runner, flow_factory=_factory, scoring_fn=_scorer)
    ...
    async def test_bootstrap_with_positive_score_passes(self):
        ...
        runner = ValidationRunner(
            task_runner=MagicMock(), flow_factory=_factory, scoring_fn=_scorer
        )
        # Patch asyncio.gather so tasks run synchronously in test
        with patch("asyncio.gather", new_callable=AsyncMock) as mock_gather:
            mock_gather.return_value = [0.75]
            result = await runner.validate(
                candidate_content="# skill", validation_task_ids=["task1"], baseline_score=None
            )
...
class TestBug002ConnectionPoolWriteLock:
    ...
    def pool(self, tmp_path):
        ...
        return SQLiteConnectionPool(db_path=tmp_path / "test.db", max_read_connections=2)
    ...
    async def test_write_lock_serialises_concurrent_writes(self, pool):
        ...
        async with pool.acquire_write() as conn:
            await conn.execute("CREATE TABLE IF NOT EXISTS t (id TEXT PRIMARY KEY, v TEXT)")
        ...
        assert not (a_start < b_start < a_end), "B must not interleave with A under the write lock"
        await pool.close()
    ...
    async def test_rollback_on_exception_leaves_clean_state(self, pool):
        ...
        await pool.initialize()
        async with pool.acquire_write() as conn:
            await conn.execute("CREATE TABLE IF NOT EXISTS t (id TEXT PRIMARY KEY)")
...
class TestBug003CircuitBreakerSleepOutsideLock:
    ...
    async def test_other_entities_not_blocked_during_stagger(self):
        ...
        async def probe_a():
            with patch.object(cb, "_maybe_stagger_probe", new_callable=AsyncMock) as mock_stagger:
        ...
        assert timeline.index("b-done") < timeline.index(
            "stagger-end"
...
class TestBug004SessionMemoryIndexPreserved:
    ...
    def _make_session(self):
        ...
        return Session(id="s1", user_id="u", agent_id="a", context=SessionContext())
    ...
    def test_memory_index_survives_add_event(self):
        ...
        assert len(plan_indices) == 1, "PlanEvent index must survive subsequent add_event() calls"
    ...
    def test_get_last_plan_uses_index_not_fallback(self):
        ...
        for i in range(5):
            session = session.add_event(MessageEvent(role="assistant", message=f"msg {i}"))
    ...
    def test_copy_is_independent(self):
        ...
        assert 99 not in mem._index["message"], "copy() must produce independent deques"
...
class TestBug005NoDoublePublish:
    ...
    async def test_event_published_once_when_flow_has_bus(self):
        ...
        state_repo.load_session = AsyncMock(
            return_value=Session(
                id="sess", user_id="u", agent_id="a", context=SessionContext(last_prompt="hello")
            )
        )
        ...
        class FakeFlow:
            ...
            _session = Session(id="sess", user_id="u", agent_id="a", context=SessionContext())
    ...
    async def test_event_published_by_runner_when_flow_has_no_bus(self):
        ...
        state_repo.load_session = AsyncMock(
            return_value=Session(
                id="sess", user_id="u", agent_id="a", context=SessionContext(last_prompt="go")
            )
        )
        state_repo.save_session = AsyncMock()
        ...
        assert len(publish_calls) == 1, "TaskRunner must publish once when flow has no event_bus"
