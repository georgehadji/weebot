...
class TestRecencyScore:
    def test_less_than_one_hour(self):
        ...
        score = _recency_score(datetime.now(UTC) - timedelta(minutes=30))
        assert score == 1.0

    def test_between_one_and_24_hours(self):
        """Between 1-24 hours → linear decay 1.0-0.3."""
        score = _recency_score(datetime.now(UTC) - timedelta(hours=6))
        assert 0.3 < score < 1.0

    def test_over_24_hours(self):
        """Over 24 hours → 0.1."""
        score = _recency_score(datetime.now(UTC) - timedelta(hours=48))
        assert score == 0.1
    ...
    def test_naive_datetime_handled(self):
        ...
        naive = datetime.now(UTC) - timedelta(hours=2)
...
class TestComputeSalience:
    def test_recent_and_frequent(self):
        ...
        score = compute_salience(
            access_count=15, last_accessed=datetime.now(UTC) - timedelta(minutes=5)
        )
        assert score >= 0.9
    ...
    def test_old_and_rare(self):
        ...
        score = compute_salience(
            access_count=1, last_accessed=datetime.now(UTC) - timedelta(days=7)
        )
        assert score <= 0.3
...
class TestLifecycleSweep:
    ...
    def mock_repo(self):
        repo = MagicMock()
        repo.get_low_salience_entries = AsyncMock(
            return_value=[
                {
                    "entry_hash": "hash_old",
