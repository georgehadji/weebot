...
class TestRateLimiter:
    ...
    def limiter(self):
        ...
        return RateLimiter(
            backend="memory", config=RateLimitConfig(requests_per_second=10.0, burst_size=5)
        )
...
class TestAuthenticator:
    ...
    def test_register_user(self, auth):
        ...
        user = auth.register_user(
            user_id="user1", name="Test User", email="test@example.com", roles={"user"}
        )
...
class TestProductionEngine:
    ...
    async def test_execute_rate_limited(self, prod_engine):
        ...
        result = await prod_engine.execute(
            "Test Template", user=Mock(id="test_user", roles={"user"})
        )
    ...
    async def test_execute_unauthorized(self, prod_engine):
        ...
        user = User(id="test_user", name="Test", email="test@example.com", roles={"readonly"})
...
class TestRedisCache:
    ...
    def test_set_and_get_template(self, mock_cache):
        ...
        template = WorkflowTemplate(name="Test", version="1.0", workflow={"task1": {}})
    ...
    def test_get_stats(self, mock_cache):
        """Cache statistics."""
        mock_cache.redis.info.return_value = {"used_memory": 1024 * 1024}  # 1 MB
        mock_cache.redis.info.return_value = {"connected_clients": 5}
    ...
    async def test_record_execution(self):
        ...
        with patch("weebot.templates.production.create_async_engine"):
            ...
            result = TemplateExecutionResult(
                success=True, template_name="Test", parameters={}, execution_time_ms=100
            )
...
class TestIntegration:
    ...
    async def test_full_production_flow(self):
        ...
        engine.authenticator.register_user("user1", "Test User", "test@example.com", roles={"user"})
