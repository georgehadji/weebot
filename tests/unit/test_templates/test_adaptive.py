...
class TestParameterSuggestions:
    ...
    async def test_suggest_parameters_no_db(self):
        ...
        template = WorkflowTemplate(name="Test", version="1.0", parameters={}, workflow={})

        from weebot.templates.adaptive import SuggestionContext

        context = SuggestionContext(
            user_id="user1", template_name="Test", template_version="1.0", previous_executions=0
        )
    ...
    async def test_suggest_parameters_no_missing_params(self):
        ...
        template = WorkflowTemplate(
            name="Test",
            version="1.0",
            parameters={"param1": ParameterSchema(name="param1", type="string")},
            workflow={},
        )
...
class TestPrivacyCompliance:
    ...
    async def test_user_hashing_in_database(self):
        ...
        engine = AdaptiveSuggestionEngine(db_session_factory=AsyncMock(), privacy_mode="strict")
    ...
    async def test_purge_old_data(self):
        ...
        engine = AdaptiveSuggestionEngine(db_session_factory=lambda: mock_session)
...
class TestFallbackBehavior:
    ...
    async def test_db_failure_returns_empty(self):
        ...
        engine = AdaptiveSuggestionEngine(db_session_factory=failing_session)
...
class TestProductionIntegration:
    ...
    async def test_adaptive_enabled_with_flag(self):
        ...
        engine = ProductionTemplateEngine(enable_adaptive=False)  # No DB, so won't actually enable
...
class TestFeatureFlags:
    ...
    def test_feature_flag_percentage_rollout(self):
        ...
        from weebot.templates.feature_flags import get_feature_flags, FeatureState, FeatureConfig

        flags = get_feature_flags()
        flags.register(
            FeatureConfig(name="test_feature", state=FeatureState.PERCENTAGE, percentage=50)
        )

        # Deterministic based on user_id
        enabled_count = sum(1 for i in range(100) if flags.is_enabled("test_feature", f"user_{i}"))
...
class TestCacheInvalidation:
    ...
    def test_cache_key_generation(self):
        ...
        key1 = engine._get_cache_key("Template1", "user1", {"param1", "param2"})
        key2 = engine._get_cache_key("Template1", "user1", {"param2", "param1"})  # Different order
    ...
    def test_cache_invalidation_on_record(self):
        ...
        # Add to cache
        cache_key = engine._get_cache_key("Template1", "user1", {"param1"})
