...
class TestPosteriorRepository:
    ...
    def repo(self, tmp_path):
        from weebot.infrastructure.persistence.posterior_repository import PosteriorRepository
...
class TestBanditSelector:
    ...
    def test_single_candidate_unchanged(self):
        ...
        result = bandit.select([_FakeCandidate("only-model", 0.8)], "test")
        assert len(result) == 1
    ...
    def test_deterministic_high_utility_wins(self):
        ...
        bandit = BanditSelector(random_seed=42)
        cands = [_FakeCandidate("good", 0.9), _FakeCandidate("bad", 0.1)]
    ...
    def test_premium_dampening(self):
        ...
        cands = [_FakeCandidate("premium-model", 0.8), _FakeCandidate("budget-model", 0.79)]
    ...
    def test_budget_tracking(self):
        ...
        for _ in range(10):
            bandit.select([_FakeCandidate("m1", 0.9), _FakeCandidate("m2", 0.8)], "track-test")
...
class TestBanditRouterIntegration:
    ...
    def test_router_with_posteriors(self):
        ...
        result = router.route("refactor the database module", posteriors=posteriors)
        assert len(result) >= 1
